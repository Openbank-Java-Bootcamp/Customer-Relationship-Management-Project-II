...
class CRMTest {
    ...
    void lookupOpportunity_ValidId_Works() {
        contact1 = testCrm.createContact(lead1);
        opportunity1 = testCrm.createOpportunity(Product.BOX, 23, contact1, salesRep1);
    ...
    public void createOpportunity_productAndContactPassed_Works(){
        ...
        int tryQuantity = 46;
        Opportunity testOpportunity = testCrm.createOpportunity(tryProduct, tryQuantity, tryContact, salesRep1);
    ...
    //SALES REP TESTS
    @Test
    public void createSalesRep_ValidName_Works() {
        Scanner testScan = new Scanner(new StringReader("Marjorie Stuart-Baxter"));
        SalesRep salesRep2 = testCrm.createSalesRep(testScan);
        assertEquals("Marjorie Stuart-Baxter", salesRep2.getName());
        assertEquals(2, testCrm.salesRepList.size());
    }

    @Test
    public void createSalesRep_InvalidName_Null() {
        Scanner testScan = new Scanner(new StringReader("3 little pigs"));
        assertEquals(null, testCrm.createSalesRep(testScan));
    }

    @Test
    public void createSalesRep_BlankName_Null() {
        Scanner testScan = new Scanner(new StringReader(" "));
        assertEquals(null, testCrm.createSalesRep(testScan));
    }

    @Test
    public void chooseSalesRep_ValidId_Works() {
        Scanner testScan = new Scanner(new StringReader("1"));
        assertEquals(salesRep1, testCrm.chooseSalesRep(testScan));
    }

    @Test
    public void chooseSalesRep_InValidId_Throws() {
        Scanner testScan = new Scanner(new StringReader("0"));
        assertThrows(NoSuchElementException.class, ()-> testCrm.chooseSalesRep(testScan));
    }

    @Test
    public void chooseSalesRep_NullId_Throws() {
        Scanner testScan = new Scanner(new StringReader(""));
        assertThrows(NoSuchElementException.class, ()-> testCrm.chooseSalesRep(testScan));
    }

    @Test
    public void showSalesRep_NotEmpty_NoThrow() {
        assertAll(()-> testCrm.showSalesReps());
    }

    @Test
    public void showSalesRep_Empty_NoThrow() {
        testCrm.salesRepList.remove("1");
        assertAll(()-> testCrm.showSalesReps());
        assertDoesNotThrow(()->testCrm.showSalesReps());
    }
}